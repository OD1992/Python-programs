...
#*****************************************************************************************************
#*************************************************************************************************
#****************************** We define the main function which contains the whole program************************************
#******************************** With a graphical interface ********************************************************
t_i=5; t_c=84; t_e=108; intercept=np.ones(t_e); h=1 
def main_program(sw_regions, sw_saturation, sw_model, sw_link): 
    if sw_regions == 'Dakar':  
        lag=[2,2,5,1] # if we applied the restriction: lag>=2 in rainfall, temperature and humidity but lag stays equal to 1 for malaria in the past;  #lag=[2,1,5,1] #Lags obtained where the correlation is max..
        y_o = df.MC_Dakar.values
        X = np.c_[df.Rainfall, AVT_Dakar, df.Humidity_D, df.MC_Dakar, intercept]
    if sw_regions == 'Fatick':
        lag=[3,4,3,1] # if we applied the restriction: lag>=2;  #lag=[3,4,3,1] ##Lags obtained where the correlation is max.
        y_o = df.MC_Fatick.values
        X = np.c_[df.Rainfall_F, AVT_Fatick, df.Humidity_F, df.MC_Fatick, intercept]      
    if sw_regions == 'Kedougou':
        lag=[2,5,2,1] # if we applied the restriction: lag>=2;  #lag=[1,5,1,1] #Lags obtained where the correlation is max.
    ...
    if sw_model==Gaussian:
        ...
        if sw_link==sqrt:
            model = sm.GLM(y_train, X_train, family=sw_model(link=sw_link)).fit()
            print("The Gaussian distribution hasn't link=sqrt")
    #print(model.summary())
    predictions = model.get_prediction(X_train)
    predictions_summary_frame = predictions.summary_frame()
    forecasted_values=forecast_functions('P1', sw_link, t_c, t_e, lag, h, model.params, X)
     #We define the ccuracy measures
    ...
    print('\\end{tabular}')
    #We show the plots
    figures('P1', y_o, t_i, t_c, t_e, lag, h, model.mu, predictions_summary_frame.iloc[:,2], predictions_summary_frame.iloc[:,2], forecasted_values, model.params, X)

#************** Calling the function with the different choices 
    #sw_regions = ['Dakar', 'Fatick', 'Kedougou']
    #sw_saturation = ['no_saturation','saturation']
    #sw_model = [Poisson, NegativeBinomial, Gaussian] 
    #sw_link=[identity, log, sqrt]
main_program(sw_regions = 'Dakar', 
                sw_saturation = 'saturation', sw_model = Poisson, 
                sw_link=identity
                )                     
