...
#************************************************************************************************************
#****************************** Ablation study: we delete one by one the explanatory variables and we experiment in the rest of data set **************************************************************
#************************** We make the experiements with the GLM Poisson only.******************************
t_i=5; t_c=84; t_e=np.shape(df)[0]; intercept=np.ones(t_e); h=1
def ablation_study(sw_regions, var_deleted, sw_link):  
    if sw_regions == 'Dakar':  
        lag=[2,2,5,1] 
    ...
    data = pd.DataFrame({'Rainfall':X[t_i-max(lag[0],h):t_c-max(lag[0],h),0], 'Temperature':X[t_i-max(lag[1],h):t_c-max(lag[1],h),1], 'Humidity':X[t_i-max(lag[2],h):t_c-max(lag[2],h),2], 'MC_past':X[t_i-max(lag[3],h):t_c-max(lag[3],h),3], 'Intercept':X[t_i-h:t_c-h,4]})
    
    forecasts = np.full(t_e, np.nan)  # set storage
    if var_deleted==['Malaria in the past']:
        X_train = data.drop('MC_past',1).values
        if sw_link==identity:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  model.params[0]*X[t-max(lag[0],h),0] + model.params[1]*X[t-max(lag[1],h),1] + model.params[2]*X[t-max(lag[2],h),2] + model.params[3]
        if sw_link==log:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  np.exp(model.params[0]*X[t-max(lag[0],h),0] + model.params[1]*X[t-max(lag[1],h),1] + model.params[2]*X[t-max(lag[2],h),2] + model.params[3])
        if sw_link==sqrt:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  (model.params[0]*X[t-max(lag[0],h),0] + model.params[1]*X[t-max(lag[1],h),1] + model.params[2]*X[t-max(lag[2],h),2] + model.params[3])**2
        ...
        plt.plot(df.Date[t_i:t_c], predictions_summary_frame.iloc[:,2], "g--", linewidth=2)
        plt.plot(df.Date[t_c:t_e], forecasts[t_c:t_e], color = "red", marker='o', markersize=8)
        plt.xlabel("Date [months]", fontsize=20)
        plt.title("A", fontsize=20)
        plt.ylabel("Malaria incidence", fontsize=20)
        ...
        plt.plot(df.Date[t_i-max(lag[1],h):t_e-max(lag[1],h)], model.params[1]*X[t_i-max(lag[1],h):t_e-max(lag[1],h),1], color="red", label="avg_temperature")
        ...
        plt.title("B", fontsize=20)
    if var_deleted==['Rainfall']:
        X_train = data.drop('Rainfall',1).values
        if sw_link==identity:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  model.params[0]*X[t-max(lag[1],h),1] + model.params[1]*X[t-max(lag[2],h),2] + model.params[2]*X[t-max(lag[3],h),3] + model.params[3]
        if sw_link==log:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  np.exp(model.params[0]*X[t-max(lag[1],h),1] + model.params[1]*X[t-max(lag[2],h),2] + model.params[2]*X[t-max(lag[3],h),3] + model.params[3])
        if sw_link==sqrt:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  (model.params[0]*X[t-max(lag[1],h),1] + model.params[1]*X[t-max(lag[2],h),2] + model.params[2]*X[t-max(lag[3],h),3] + model.params[3])**2  
        #Plots
        predictions = model.get_prediction(X_train)
        predictions_summary_frame = predictions.summary_frame()
        plt.figure(figsize=(12,4))
        ...
        plt.legend(["Malaria incidence count", "predicted data", "mean_ci_lower", "mean_ci_upper", "forecasted data"])
        plt.subplot(122)
        plt.plot(df.Date[t_i-max(lag[1],h):t_e-max(lag[1],h)], model.params[0]*X[t_i-max(lag[1],h):t_e-max(lag[1],h),1], color="red", label="avg_temperature")
        ...
        plt.plot(df.Date[t_i-max(lag[3],h):t_e-max(lag[3],h)], model.params[2]*X[t_i-max(lag[3],h):t_e-max(lag[3],h),3], color="black", label="Malaria in the past")
        plt.title("B", fontsize=20)
    if var_deleted==['Temperature']:
        X_train = data.drop('Temperature',1).values
        if sw_link==identity:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  model.params[0]*X[t-max(lag[0],h),0] + model.params[1]*X[t-max(lag[2],h),2] + model.params[2]*X[t-max(lag[3],h),3] + model.params[3]
        if sw_link==log:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  np.exp(model.params[0]*X[t-max(lag[0],h),0] + model.params[1]*X[t-max(lag[2],h),2] + model.params[2]*X[t-max(lag[3],h),3] + model.params[3])
        if sw_link==sqrt:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  (model.params[0]*X[t-max(lag[0],h),0] + model.params[1]*X[t-max(lag[2],h),2] + model.params[2]*X[t-max(lag[3],h),3] + model.params[3])**2           
        predictions = model.get_prediction(X_train)
        predictions_summary_frame = predictions.summary_frame()
        #Plots
        plt.figure(figsize=(12,4))
        ...
        plt.plot(df.Date[t_i-max(lag[3],h):t_e-max(lag[3],h)], model.params[2]*X[t_i-max(lag[3],h):t_e-max(lag[3],h),3], color="black", label="Malaria in the past")
        plt.title("B", fontsize=20)
    if var_deleted==['Humidity']:
        X_train = data.drop('Humidity',1).values
        if sw_link==identity:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  model.params[0]*X[t-max(lag[0],h),0] + model.params[1]*X[t-max(lag[1],h),1] + model.params[2]*X[t-max(lag[3],h),3] + model.params[3]
        if sw_link==log:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  np.exp(model.params[0]*X[t-max(lag[0],h),0] + model.params[1]*X[t-max(lag[1],h),1] + model.params[2]*X[t-max(lag[3],h),3] + model.params[3])
        if sw_link==sqrt:
            model = sm.GLM(y_train, X_train, family=Poisson(link=sw_link)).fit()
            for t in np.arange(t_c,t_e):
                forecasts[t] =  (model.params[0]*X[t-max(lag[0],h),0] + model.params[1]*X[t-max(lag[1],h),1] + model.params[2]*X[t-max(lag[3],h),3] + model.params[3])**2
        predictions = model.get_prediction(X_train)
        predictions_summary_frame = predictions.summary_frame()
        #Plots
        plt.figure(figsize=(12,4))
    ...
    print('\\hline')
    accuracy_measures(sw_regions,sw_model,sw_link,'each_link',model.mu, forecasts[t_c:t_e], y_train, y_test, t_i, t_c, t_e, y_o, min(model.mu))
    print('\\end{tabular}')


#*********** Calling the function by taking the following input**********************    
    #sw_regions = ['Dakar', 'Fatick', 'Kedougou']
    #var_deleted=[['Malaria in the past'], ['Rainfall'], ['Temperature'], ['Humidity']]
    #sw_link=[identity, log, sqrt]
ablation_study(sw_regions ='Dakar',
                var_deleted=['Malaria in the past'],
                sw_link=identity
                )
